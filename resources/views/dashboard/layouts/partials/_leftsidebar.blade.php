...
<aside class="main-sidebar sidebar-dark-primary elevation-4 text-md">
    ...
    <a href="{{ route('dashboard.home') }}" class="brand-link">
        <img src="{{ asset('images/lsdm-logo-02.jpg') }}" alt="LSDM" class="brand-image img-circle elevation-3"
            style="opacity: .8">
        <span class="brand-text font-weight-bold h6">LSDM</span>
    </a>
    <!-- Sidebar -->
    <div class="sidebar">
        <!-- Sidebar Menu -->
        <nav class="mt-4">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu"
                ...
                @auth
                    ...
                    @hasrole('owner|admin')
                        {{-- Admins Management --}}
                        <li class="nav-item {{ isMenuOpen(['users', 'roles', 'permissions', 'logs', 'notifications']) }} mt-2">
                            ...
                            <ul class="nav nav-treeview">
                                ...
                                @endif
                                @if (auth()->user()->can('notifications.index'))
                                    <li class="nav-item">
                                        <a href="{{ route('notifications.index') }}"
                                            class="nav-link {{ isRouteActive('notifications*') }}">
                    ...
                    @hasanyrole('admin|editor')
                        @if (auth()->user()->can('posts.index'))
                            <li class="nav-item">
                                <a href="{{ route('posts.index') }}" class="nav-link {{ isRouteActive('posts*') }}">
